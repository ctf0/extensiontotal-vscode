...
export async function activate(context: vscode.ExtensionContext) {
  ...
  const scanHandler = async (forceScanAll: boolean = false, waitForInterval: boolean = false) => {
    ...
    if (waitForInterval) {
      ...
      const lastScan = context.globalState.get(`last-extensiontotal-scan`, null);
      const now = new Date().getTime();

      if (
        scanInterval !== 0 &&
        lastScan &&
        lastScan < (now - (scanInterval * 60 * 60 * 1000))
      ) {
        return;
      }

      context.globalState.update(`last-extensiontotal-scan`, now);
    }
  ...
  vscode.extensions.onDidChange(
    debounce(async () => await scanHandler(false, true), 10 * 1000),
  );
